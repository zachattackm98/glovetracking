/*
  # Initial schema setup for asset tracking

  1. New Tables
    - `assets`: Stores asset information including certification status
    - `certification_documents`: Stores document records for asset certifications
...
CREATE POLICY "Admins can manage all assets in their organization"
  ...
  TO authenticated
  USING (
    org_id = (auth.jwt() ->> 'org_id')::text AND
...
CREATE POLICY "Members can view their assigned assets"
  ...
  USING (
    org_id = (auth.jwt() ->> 'org_id')::text AND
    (
      assigned_user_id = auth.uid()::text OR
      (auth.jwt() ->> 'org_role')::text = 'org:admin'
    )
  );
